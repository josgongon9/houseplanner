...
import { generateDueRecurringExpenses } from '../lib/recurringExpenses';
...
export const StoreProvider = ({ children }) => {
    ...
    const generateRecurringExpenses = useCallback(async (monthDate = new Date()) => {
        if (!user?.uid || !household?.id) return;
        await generateDueRecurringExpenses(household.id, monthDate);
    }, [user?.uid, household?.id]);

    useRecurringSchedule(generateRecurringExpenses, Boolean(user?.uid && household?.id), recurringExpenses);
